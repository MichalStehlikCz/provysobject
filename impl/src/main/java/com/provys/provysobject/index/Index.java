...
public interface Index<V extends ProvysObjectValue, P extends ProvysObjectProxy<?, V>> {
    ...
    /**
     * Called when proxy value changes. Marks group that originally contained proxy as incomplete. Used both on update
     * and on load / unload of data to cache
     *
    ...
    /**
     * Called when object is deleted or it no longer belongs to given sub-index as its position of index has been
     * changed. Completeness of data is not affected (unlike when new value would be changed to null using update).
     * Indices do not maintain data for not loaded items, thus if old value is null, notification is not sent to index
     *
     ...
     * save space. Any further queries using unique id will be still performed against index first and only misses go
