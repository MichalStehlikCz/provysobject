...
class ProvysNmObjectManagerImplTest {
  ...
  @Test
  void getOptionalByNameNmTest() {
    var repository = mock(ProvysRepository.class);
    var loader = spy(new TestNmObjectLoaderImpl());
    var manager = new TestNmObjectManagerImpl(repository, loader);
    var nameNm5 = "NM5";
    var proxy5 = manager.getOptionalByNameNm(nameNm5).orElseThrow();
    assertThat(proxy5.getNameNm()).isEqualTo(nameNm5);
    assertThat(manager.getOptionalByNameNm(nameNm5)).containsSame(proxy5);
    assertThat(manager.getById(proxy5.getId())).isSameAs(proxy5);
    var idValue1 = DtUid.valueOf("1");
    var proxy1 = manager.getById(idValue1);
    assertThat(proxy1.getId()).isEqualTo(idValue1);
    assertThat(manager.getById(proxy5.getId())).isSameAs(proxy5);
    assertThat(manager.getOptionalByNameNm(proxy1.getNameNm())).containsSame(proxy1);
    verify(loader, times(1)).loadByNameNm(manager, nameNm5);
    verify(loader, times(1)).loadById(manager, idValue1);
    verify(loader, times(1)).loadByNameNm(any(), any());
    verify(loader, times(1)).loadById(any(), any());
    assertThat(manager.getOptionalByNameNm("NAME8")).isEmpty();
  }
  ...
  void registerChange() {
    ...
    var value5 = new TestNmObjectValue(DtUid.valueOf("5"), "NM5", "text");
    ...
    // to be loaded
    manager.unregister(proxy5, proxy5.getValueObject());
    assertThat(manager.getByNameNm("NM5")).isNotSameAs(proxy5)
        .isEqualTo(proxy5); // after un-registration, we
